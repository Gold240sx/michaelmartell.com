...
import {
	...
	createAccountViaDiscord,
	createAccountViaApple,
	updatePassword,
...
// Define interfaces for OAuth providers
...
export async function createDiscordUserUseCase(discordUser: DiscordUser) {
	let existingUser = await getUserByEmail(discordUser.email)

	if (!existingUser) {
		existingUser = await createUser(discordUser.email)
	}

	await createAccountViaDiscord(existingUser.id, discordUser.id)
...
export async function createAppleUserUseCase(appleUser: AppleUser) {
	let existingUser = await getUserByEmail(appleUser.email)

	if (!existingUser) {
		existingUser = await createUser(appleUser.email)
	}

	await createAccountViaApple(existingUser.id, appleUser.sub)

	// Apple doesn't provide a username or avatar, so we'll generate a random name
	const displayName = uniqueNamesGenerator({
		dictionaries: [colors, animals],
		separator: " ",
		style: "capital",
	...
	await createProfile(existingUser.id, displayName)
