...
export async function createAccountViaApple(userId: UserId, appleId: string) {
	await database
		.insert(accounts)
		.values({
			userId: userId,
...
export async function getAccountByAppleId(appleId: string) {
	return await database.query.accounts.findFirst({
		where: eq(accounts.appleId, appleId),
	})
}
